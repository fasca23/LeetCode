...
description: Дано число numRows. Построить первые numRows строк треугольника Паскаля. Каждый элемент — сумма двух верхних. Итеративное построение ряд за рядом.
screenshots: 0
---

**Подход: Итеративное построение ряд за рядом**

1. **Идея:** Первый ряд = [1]. Каждый следующий: по краям 1, в середине `row[j] = prev[j-1] + prev[j]`. Повторяем numRows раз.
...
3. **Время:** O(n²) — общее число элементов = n(n+1)/2.
